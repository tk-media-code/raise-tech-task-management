...
4. **コミット → push → PR作成。** PR本文に `Closes #<issue番号>` を必ず含める
5. **マージはマージコミット方式で行う。** `gh pr merge --merge` を使う。Squash・Rebaseは使わない（GitHub側でもマージコミット以外は選択できないよう設定済み）
6. **マージ後は後片付けをする。** リモート・ローカル両方の作業ブランチを削除する（`gh pr merge --delete-branch` ならリモートは同時に削除される）。Issueは `Closes` キーワードによりマージと同時に自動closeされる
