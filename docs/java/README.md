...
カード・ボードの新規作成（POST）とBean Validationの導入（[docs/spring-boot 29章](../spring-boot/09-write-api-validation.md#29-リクエストdtoとbean-validation)）を実装しましたが、このリストの機能はいずれも必要になりませんでした（アノテーションがrecordのどこへ伝播するかという新しい話題は、既存の[29章](./07-syntax-reference.md#29-アノテーションの読み方構文として)への追記で収まっています）。残るPUT/DELETEの実装が進むにつれて、これらの機能が登場する可能性はまだあります。実装に登場した時点で、下記の更新ルールに従ってこのドキュメント群に章を追加してください。
