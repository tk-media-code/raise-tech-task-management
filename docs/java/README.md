...
| 26〜29章 | 構文リファレンス（文字列・日付時刻・演算子・アノテーション） | [07-syntax-reference.md](./07-syntax-reference.md) |
...
| 配列（`String[] args`以外） | データの集まりはすべて`List`・`Map`（[19章](#19-コレクション)）で表現しています |
| 可変長引数（`...`） | 引数の個数が可変になるメソッドがまだ存在しません |
