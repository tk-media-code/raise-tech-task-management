...
> 実装上、削除ボタンはこのモーダルには置いていない。完全削除できるのはアーカイブ済みのカードのみとしたため（[5.7](./02-requirements.md#57-アーカイブ)）、①・③など通常のボード表示から開いたこのモーダルでは常に押せないボタンになってしまう。削除の導線はアーカイブ画面の各行（⑥の[完全削除]）に一本化している。上のワイヤーフレームの[削除]は、この判断が定まる前の初期案として残す。
