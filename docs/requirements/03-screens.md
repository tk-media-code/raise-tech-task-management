...
スマートフォン表示では上部の「未着手／作業中／完了」タブでステータスを切り替え、選択中のステータスのカードだけを1列で表示する。ドラッグ操作に加えて各カードに「移動」ボタンを設け、ボタン操作だけでもステータスを変更できるようにする（[5.3](./02-requirements.md#53-ドラッグドロップによるステータス変更)、[8.1](./02-requirements.md#81-対象デバイス画面サイズ)）。
...
タイトル・ステータス・説明・期日・ラベルを1画面で編集でき、アーカイブもここから行える。

> 削除ボタンはこのモーダルには置いていない。完全削除できるのはアーカイブ済みのカードのみとしたため（[5.7](./02-requirements.md#57-アーカイブ)）、①・③など通常のボード表示から開いた場合は常に押せないボタンになってしまう。削除の導線はアーカイブ画面の各行（⑥の[完全削除]）に一本化している。
...
完了カードをアーカイブすると①・③の表示からは消え、この画面にのみ残る。各カードから元のボードへの「復元」または「完全削除」ができる。完全削除は取り消せないため、実行前に確認ダイアログを表示する。
