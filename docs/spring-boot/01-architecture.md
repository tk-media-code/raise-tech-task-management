...
| Repository（データアクセス）   | 実装済み（GET・カード/ボード登録・カード更新・ボード更新/削除）      | `repository/BoardRepository.java` 等（[17〜19章](./05-repository.md)参照）                                     |
| Service（ビジネスロジック）    | 実装済み（GET・カード/ボード登録・カード更新・ボード更新/削除）      | `service/BoardService.java` 等（[20章](./06-service-controller.md#20-service層とtransactional)、[31章](./09-write-api-validation.md#31-登録処理の中身)、[34〜36章](./10-update-api.md)、[39章](./10-update-api.md#39-ボードの改名と並べ替え)、[40〜41章](./11-delete-api.md)参照）           |
| Controller（API）              | 実装済み（GET・カード/ボード登録・カード更新・ボード更新/削除）      | `controller/BoardController.java` 等（[21章](./06-service-controller.md#21-controller層とrest-api)、[28章](./09-write-api-validation.md#28-登録系apipostの作り方)、[33章](./10-update-api.md#33-更新系apiputpatchの作り方)、[40章](./11-delete-api.md#40-削除apideleteと204-no-content)参照）       |
| DTO（レスポンス用・リクエスト用） | 実装済み                             | `dto/BoardResponse.java`・`dto/CardCreateRequest.java`・`dto/CardUpdateRequest.java` 等（[22章](./06-service-controller.md#22-dtoレコードでエンティティを外に出さない)、[29章](./09-write-api-validation.md#29-リクエストdtoとbean-validation)参照） |
...
現時点ではRead系（GET）・カード/ボードの新規登録（POST）に加え、カードの編集・ステータス変更・アーカイブ（PUT/PATCH、[33〜38章](./10-update-api.md)）、ボードの改名・並べ替え（PUT/PATCH、[39章](./10-update-api.md#39-ボードの改名と並べ替え)）・削除（DELETE、[40〜41章](./11-delete-api.md)）が実装済みです。カードの完全削除といった残りのWrite系API（DELETE）や、独自ロジックに対する自動テストの導入が今後の課題になります。新しい層・概念を実装した際は、[README.mdの更新ルール](./README.md#このドキュメントの更新ルール)に従ってこのドキュメント群を更新してください。
