...
| Repository（データアクセス）   | 実装済み（GET・カード/ボード登録・カード更新）      | `repository/BoardRepository.java` 等（[17〜19章](./05-repository.md)参照）                                     |
| Service（ビジネスロジック）    | 実装済み（GET・カード/ボード登録・カード更新）      | `service/BoardService.java` 等（[20章](./06-service-controller.md#20-service層とtransactional)、[31章](./09-write-api-validation.md#31-登録処理の中身)、[34〜36章](./10-update-api.md)参照）           |
| Controller（API）              | 実装済み（GET・カード/ボード登録・カード更新）      | `controller/BoardController.java` 等（[21章](./06-service-controller.md#21-controller層とrest-api)、[28章](./09-write-api-validation.md#28-登録系apipostの作り方)、[33章](./10-update-api.md#33-更新系apiputpatchの作り方)参照）       |
