...
アーカイブ済みのカードは、ボード詳細にも検索結果にも表示されません（どの画面も`archived=false`で取得しているためです）。しかし「画面に導線が無い」ことと「APIがその操作を拒否する」ことは別の話です。`PATCH /api/cards/{id}/status`を直接呼べばステータスを変えられてしまうと、[38章](#38-アーカイブフラグ更新と冪等性)が前提にしている「アーカイブされているカードの`status`は必ず`"done"`のまま保たれる」という不変条件が崩れます。doneのままアーカイブしたカードをtodoに変えてから復元すると、「アーカイブしたはずのカードが未着手になって戻ってくる」ことになり、要件5.7の「元のステータスへ復元できる」が意味を失います。

そこで、カードを取得した直後に`isArchived`を確認し、アーカイブ済みであれば400で弾きます。これは[38章](#38-アーカイブフラグ更新と冪等性)の「完了ステータスのカードのみアーカイブできる」・[42章](./11-delete-api.md#42-削除の可否を状態で決める在るかだけでは足りないとき)の「アーカイブ済みのカードのみ削除できる」と同じ性格のチェックです。いずれも**対象カードの現在の状態**に依存する制約で、リクエストの中身だけを見るBean Validationでは表現できないため、Service層の入り口で検証します。3つのメソッドが揃って同じ形のチェックを持つことにより、「アーカイブ済みのカードが受け付けるのは、復元と完全削除だけ」という状態遷移が、どの経路から来ても破られなくなります。

なお、フロントエンド側でもカード詳細モーダルのステータス選択欄を`disabled`にしています（アーカイブ画面からもこのモーダルは開けるためです）。ただしそれは「操作させない親切」であって制約の実体ではなく、ルールを保証しているのはあくまでこのService層のチェックです。
...
この`if`が`archived`（＝アーカイブする方向）のときにしか出てこないのは、復元（`archived == false`）には対応する制約が無いためです。アーカイブ処理が`status`を一切変更しない（前節参照）ことに加え、[36章](#36-ステータス変更と列内の並び替え)の`updateStatus`がアーカイブ済みカードへの変更を拒否することにより、アーカイブされているカードの`status`は必ず`"done"`のまま保たれます。したがって「アーカイブされているカードを復元してよいか」を判定するための追加チェックはそもそも不要で、復元は常に許可されます。
