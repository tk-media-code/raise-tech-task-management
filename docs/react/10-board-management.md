...
> **⚠️ この判断は[38章](./12-dialog-accessibility.md#38-共通の確認ダイアログwindowconfirmをやめてネイティブdialogに寄せる)で覆しました。** 上の理由づけ自体は正しいのですが、「標準ダイアログは必ず表示される」という暗黙の前提が置かれていました。実際には`window.confirm()`はブラウザ側の設定で抑制でき、抑制されると**ダイアログを出さずに即座に`false`を返す**ため、アプリ側からは「キャンセルされた」のか「表示されなかった」のかを区別できません。確認が出ないまま削除される事故が実際に起きたため、現在は共通コンポーネント`ConfirmDialog`（ネイティブ`<dialog>`）に置き換えています。当時の判断とその前提を、経緯としてこのまま残しておきます。
