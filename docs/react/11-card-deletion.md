...
> **⚠️ その`window.confirm()`は[38章](./12-dialog-accessibility.md#38-共通の確認ダイアログwindowconfirmをやめてネイティブdialogに寄せる)で共通コンポーネント`ConfirmDialog`（ネイティブ`<dialog>`）へ置き換えました。** ブラウザ側の設定で標準ダイアログが抑制されると、確認が出ないまま削除される状態になり得たためです。ただし上の「巻き込むものが無いから件数を数えない」という判断はそのまま有効で、置き換え後も事前GETは行っていません。
...
> **📌 その後の経緯：`<dialog>`の入れ子は[38章](./12-dialog-accessibility.md#38-共通の確認ダイアログwindowconfirmをやめてネイティブdialogに寄せる)で実際に確かめ、採用しました。** ボード削除の確認ダイアログが`BoardManageModal`（`<dialog>`）の中に置かれるため、避けて通れなくなったためです。結果として②は「未検証だから避ける」案ではなくなりましたが、**この`LabelPicker`のインラインUIはそのまま残しています**。もともと`window.confirm()`を使っておらず38章のきっかけとなった不具合の対象外であることに加え、削除対象のチップのすぐ下に出ること自体が「どのラベルを消すのか」を示す手がかりになっているためです。共通化それ自体が目的ではない、という判断は変わっていません。
