...
かつて`components/BoardSelect.tsx`と`components/LabelFilterBar.tsx`は、まったく同じ`GET /api/boards`をそれぞれ独立して呼び出していました。当時のコメントは、その方針をこう説明しています。

```typescript
// （当時のLabelFilterBar.tsx。現在この取得は行いません）
...
「取得するだけ（Read）のコンポーネントが複数あるだけなら、それぞれが自分で取りに行けばよい」という方針です。これは**2段階を経て、最終的に全面的に取り下げられました**。理由がそれぞれ異なるため、順に見ていきます。

#### 第1段階：Write→Readの関係が生まれて崩れた

ボード管理モーダルでボードを新規作成したとき、その結果はヘッダーのセレクトボックス（`BoardSelect`）の選択肢にも反映される必要があります（要件5.1）。`BoardSelect`が独立して`useApi(apiPaths.boards())`を呼び続ける方針のままでは、モーダル側の変更をセレクトボックスへ伝える手段がありません。「必要なコンポーネントがそれぞれ独立して取りに行く」という方針は、**取得するだけ**（Read）のコンポーネントが複数あるだけなら問題になりませんが、**一方が書き込みを行い、その結果をもう一方が知る必要がある**（Write→Read）関係が生まれた時点で成立しなくなります。
...
`BoardSelect`のリフトアップによって、`App.tsx`は常に最新のボード一覧を持つようになりました。この時点で、`LabelFilterBar`が自分でもう一度`GET /api/boards`を呼ぶ理由は無くなっています。第1段階のような書き込みとの競合こそ起きないものの、**すぐ上の親がすでに持っている値を、わざわざもう一度サーバーへ取りに行っている**だけの状態です。

しばらくは「独立取得の方針を踏襲している箇所」として残っていましたが、正直に言えばこれは方針というより**揃え忘れ**でした。`App.tsx`のdocblockは「ボード一覧のAPIをアプリ起動あたり1回叩くだけで済む」と明記しており、`LabelFilterBar`はその記述に対する唯一の反例にもなっていました。検索画面を開くたびに2回目の`GET /api/boards`が飛ぶのをやめ、`App.tsx` → `SearchView` → `LabelFilterBar`とpropsでリレーする形に統一しています。

```typescript
// App.tsx（SearchView自身はboardsを使わない。配下のLabelFilterBarへ中継させるために渡す）
<Route
  path="/search"
  element={<SearchView boards={boards} boardsLoading={boardsLoading} boardsError={boardsError} />}
/>
```

中継のためだけに3つのpropsが`SearchView`を素通りする形になりました。これは俗に**prop drilling（propsのバケツリレー）**と呼ばれ、Contextの導入がしばしば検討される兆候です。今回は経由するのが1階層だけなので、まだリレーで十分と判断しています。

なお、この統一は「独立した取得そのものが悪い」という話ではありません。`LabelFilterBar`は今もラベル一覧（ボードごとの`GET /api/boards/{id}/labels`）を`useLabelsByBoard`で自分で取得しています。ラベルを必要とするのはこの画面だけで、親が持っているわけでも、他のコンポーネントの書き込みと競合するわけでもないためです。**「親がすでに持っているか」「書き込みとの競合があるか」という2点が、独立して取りに行ってよいかの判断材料**になります。

**Contextには飛びつかなかった**：Reactには、離れたコンポーネント同士でデータを共有する**Context**という仕組みがありますが、今回もまだ採用していません。ボード一覧を必要とするのは`BoardSelect`・`BoardManageModal`・`CrossBoardView`・`LabelFilterBar`（`SearchView`経由）で、いずれも`App.tsx`から1〜2階層の距離にあり、propsで配るだけでコードの見通しは十分に保てます。Contextを持ち込むと、「この値はどこから来るのか」を`useContext`の呼び出し元だけを見ても追えなくなり（Providerの位置まで遡る必要がある）、消費者が少ない今の規模では見合わないコストです。「独立した取得」が成立しなくなったからといって、次の手が必ずContextとは限りません——**まず親へのリフトアップで足りないか**を検討し、それでも足りなくなったとき（消費者がさらに増える、あるいは画面をまたいで深くネストする、など）に初めてContextを検討する、という順序で判断しています。
...
| データをどこで取得するか | 「親がすでに持っているか」「他方の書き込みと競合するか」で判断する。どちらも無ければ独立して取得してよい。どちらかに当てはまったら共通の親へリフトアップする | ボード一覧＝`App.tsx`へリフトアップ済み（`BoardSelect`・`BoardManageModal`・`CrossBoardView`・`LabelFilterBar`へ配布、[19章](./08-form-and-mutation.md#19-書き込みpostとデータの更新)）／ ラベル一覧＝`LabelFilterBar`が独立して取得 |
