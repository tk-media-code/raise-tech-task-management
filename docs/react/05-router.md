...
`*`（ワイルドカード）は、あらゆるパスに一致する特別な`path`です。`<Routes>`が「一致した**最初の1つだけ**を描画する」という前節の性質と組み合わさることで、「上のどのルートにも一致しなかったとき」だけこれが選ばれます。裏を返すと、**書く位置が意味を持つ**唯一の`<Route>`でもあります。先頭に置いてしまうと、すべてのURLがここで止まり、他のルートには永久に到達しません。

このルートが無いとどうなるかは、実際に`/typo`のような存在しないURLを開くと分かります。`<Routes>`は一致する`<Route>`を見つけられず、**何も描画しません**。エラーにはならず、ヘッダーだけが残った空の`<main>`が表示されます。ユーザーからは「アプリが壊れた」のか「そんなURLは無い」のかを区別できず、しかも自力で戻る手段（リンク）も画面上にありません。存在しないURLへは、古いブックマーク・共有された壊れたリンク・単なる打ち間違いといった経路で日常的に到達し得るため、その受け皿を1行用意しておく価値があります。

なお、`path="*"`が拾うのは「フロントエンドのルーティングに到達した後、どのルートにも一致しなかった」ケースだけです。この判定はブラウザ上のReactが行うものであり、バックエンド（Spring Boot）が返す404（存在しないカードIDを指定した場合など）とは別の層の話です。後者は各ページが`useApi`のエラーとして受け取り、それぞれの画面の中で「読み込みに失敗しました」として表示します。
