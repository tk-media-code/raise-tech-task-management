...
import type { CardCreateRequest, CardResponse, LabelResponse } from '../types/api'
...
function CardCreateForm({ boardId, onCreated }: Props) {
  ...
  const { data: labels } = useApi<LabelResponse[]>(open ? apiPaths.boardLabels(boardId) : null)
  ...
  return (
    ...
    >
      ...
      {/* ラベルが1件も無いボードでは、選ぶものが無い選択UIを出しても意味が無いため表示しない */}
